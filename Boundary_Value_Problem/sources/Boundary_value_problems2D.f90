...
!logical, allocatable :: dUs(:,:) 

!logical :: dU(3, 2) ! matrix of dependencies( number of variables, direction, order ) 
...
!
!**************************************************************************************************************
subroutine Boundary_Value_Problem2D( x_nodes, y_nodes, Order, Differential_operator, Boundary_conditions, Solver, Solution)
     ...
     linear2D = Linearity_BVP_2D( Differential_operator ) 
    
     if (linear2D) then 
        
              call Linear_Boundary_Value_Problem2D(x_nodes, y_nodes, Order, Differential_operator,  Boundary_conditions, Solution)         
     else 
              call Non_Linear_Boundary_Value_Problem2D( x_nodes, y_nodes, Order, Differential_operator, &
...
!
!**************************************************************************************************************
subroutine Linear_Boundary_Value_Problem2D(x_nodes, y_nodes, Order, Differential_operator,  Boundary_conditions, Solution)
...
!  *** variables specification
       ...
       dUs = Dependencies_BVP_2D( Differential_operator ) 
...
subroutine Difference_equation(x, y, W, Wx, Wy, Wxx, Wyy, Wxy, Fxy) 
    ...
    integer :: i0(2), iN(2), di(2), j0(2), jN(2), dj(2)    
     ...
     j0 = 0; jN = Ny; dj = [ Ny, 1 ]  
      ...
      if ( dUs(5) ) call Derivative( ["x","y"], 2, 1, Wx, Wxy)
...
!
!**************************************************************************************************************
subroutine Non_Linear_Boundary_Value_Problem2D( x_nodes, y_nodes, Order, Differential_operator, &
                                                          Boundary_conditions, Solver, Solution)
...
!
!**************************************************************************************************************
subroutine Boundary_Value_Problem2D_system(x_nodes, y_nodes, Order, N_variables, Differential_operator,  Boundary_conditions, Solution, Solver)
     ...
     linear2D = Linearity_BVP_2D_system( N_variables, Differential_operator )  
    
     
     if (linear2D) then 
         
              call Linear_Boundary_Value_Problem2D_system(x_nodes, y_nodes, Order, N_variables, Differential_operator,  Boundary_conditions, Solution)
     else 
              call Non_Linear_Boundary_Value_Problem2D_system( x_nodes, y_nodes, Order, N_variables, Differential_operator, &
                                                               Boundary_conditions, Solver, Solution)         
     end if 
...
!
!**************************************************************************************************************
subroutine Linear_Boundary_Value_Problem2D_system(x_nodes, y_nodes, Order, N_variables, Differential_operator,  Boundary_conditions, Solution)
...
!  *** variables specification
       ...
       logical ::  dUs(N_variables, 5)  ! matrix of dependencies( variable, derivative )
       
       dUs = Dependencies_BVP_2D_system( N_variables, Differential_operator ) 
   ...
   !          ijk = k + i*Nv + j * (Nx+1)*Nv
     ...
     !        Difference_operator(1:M,ijk) = reshape( F, [ M ])
         !     Difference_operator(1:M,ijk) = reshape_to_vector( Nx, Ny, Nv, F )  
              Difference_operator(1:Ns,ijk) = reshape_to_vector( F )   
...
subroutine Difference_equation(x, y, W, Wx, Wy, Wxx, Wyy, Wxy, Fxy) 
    ...
    integer :: i0(2), iN(2), di(2), j0(2), jN(2), dj(2)    
     ...
     do i=1, Nv 
       ...
       if ( dUs(i,5) ) call Derivative( ["x","y"], 2, 1, Wx(0:,0:,i), Wxy(0:,0:,i))
     end do   
...
! 
! !------------------------------------------------------------
!function  reshape_F_to_vector(  ) result(V) 
!    real :: V( (Nx+1)*(Ny+1)*Nv )  
!    
!      integer :: i1, j1, k1, ijk1  
!      
!      do i1=0, Nx
!          do j1=0, Ny 
!              do k1=1, Nv 
!                  ijk1 =  v_index(i1, j1, k1)
!                  V(ijk1) = F(i1,j1,k1)
!              end do 
!          end do 
!      end do
...
!    
!          
!    v_index  = i + 1 + j*(Nx+1) + (Nx+1)*(Ny+1)*(k-1) 
!  
!    
!end function 
! 
!------------------------------------------------------------
 ...
 !------------------------------------------------------------
function  reshape_to_vector( A ) result(V) 
...
!
!**************************************************************************************************************
subroutine Non_Linear_Boundary_Value_Problem2D_system( x_nodes, y_nodes, Order, N_variables, Differential_operator, &
                                                          Boundary_conditions, Solver, Solution)
